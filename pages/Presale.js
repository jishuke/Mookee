...
export default class Presale extends Component{
	constructor(props){
		super(props);
		this.state = {
			title:I18n.t('MyScreen.PreSaleOrder') ,
			diy_data: [],//存放所有的装修改数据
			showWait: false,
			refresh: false,
			data: [],//
	...
	getDiyData = () => {
		RequestData.getSldData(AppSldUrl + '/index.php?app=index&mod=index_data&sld_addons=presale'+this.extend_param_str).then(res => {
			if(res.code == 200){
	...
	// 获取预售分类
	getPresaleCat(){
		RequestData.getSldData(AppSldUrl + '/index.php?app=goods&mod=getclasslist&sld_addons=presale').then(res => {
			if(res.status == 200){
	...
	render(){
		const {diy_data, title, CatList, class_id} = this.state;
		return (
