...
import AutoHeightWebView from '../src/ahwv/AutoHeightWebView'
...
const gifViewStyles = StyleSheet.create({
	container:{
		...
		alignItems: 'center',
		position: 'absolute',
		top: Platform.OS === 'ios' ? (Dimensions.get('window').height === 812 || Dimensions.get('window').height === 896) ? 88 : 64 : pxToDp(50),
		bottom: 0,
		left: 0,
		right: 0,
		zIndex: 3,
...
export default class GoodsDetailNew extends Component {
	...
	_renderDetail(data) {
		if (data && data.length > 0) {
			let source = {html: data};
			if (Platform.OS === 'android') {
				source.baseUrl = '';
			}
			//console.log('详情数据:', format + data)
	...
	componentDidMount(){
		...
		this.props.navigation.addListener("willFocus", payload => {
			// console.warn('ww:GoodsDetail:willFocus');
            StorageUtil.get('language', (error, object) => {
                if (!error && object) {
                    this.setState({
                        language: object
	...
	getSldGoodsBody = (gid) => {
		return new Promise((resolve, reject)=>{
			RequestData.getSldData(AppSldUrl + '/index.php?app=goods&mod=goods_body_xcx&gid=' + gid)
				.then(result => {
					if(result.state == 200){
						console.log('商品详情富文本00000000：', result)
						let body = result.goods_detail.goods_body;
						let reg = new RegExp('<img', "g")
                        console.log('商品详情富文本111111111：', result)
						this.setState({
...
//处理输入商品数量
	...
	//加入购物车事件,无key的话存本地缓存
	addCart = () => {
		let { buy_num, goods_info, gid, refeCode } = this.state
	...
	goBuy = (type = '', is_join = '') => {
        ...
        // }
        // return false
		const {goods_detail, sele_pintuan_team_id, refeCode, language} = this.state;
		if(key){
			if(!this.seleSpecing){
				this.refs.seleSpec.open();
				this.seleSpecing = 1;
	...
	render(){
		...
		const {
			...
			showManSong, promotion_type, presaleInfo, pinladder, pin_info, is_show_default_btn, video_url,currentIndex,modalVisible,images, language
		} = this.state;

        console.log('用户评价人数：',  evaluateinfo ? evaluateinfo.all : '')
		...
		return (
			<View style={ [ styles.container, {paddingBottom: 50, paddingTop: Platform.OS === 'ios' ? (height === 812 || height === 896) ? 44 : 20 : StatusBar.currentHeight} ] }>
				{/* header */}
				...
				<View style={{flex: 1}}>
                    ...
                    >
                        ...
                        { goods_info.promotion_type == 'tuan' && (
                            <ImageBackground
								source={ require("../assets/images/sld_gdetail_tuan.png") }
								resizeMode="stretch"
							...
							>
                                ...
                                <View style={ {
                                    ...
                                    <Text style={ {
                                        ...
                                        {I18n.t('GoodsDetailNew.dumpling')} { goods_info.saled_num }{I18n.t('GoodsDetailNew.piece')}
									</Text>
                                    <View style={[GlobalStyles.flex_common_row, {marginTop: 3}]}>
                                        <View style={[GlobalStyles.flex_common_row, {marginLeft: 4}]}>
                                            <Text style={{
                                                color: '#F31367',
                                                fontSize: 10
                                            }}>
                                                {(new Date(goods_info.promotion_start_time_stamp * 1000)) - new Date() > 0 ? I18n.t('GoodsDetailNew.starttime') : ((new Date(goods_info.promotion_end_time_stamp * 1000)) - new Date() > 0 ? I18n.t('GoodsDetailNew.endtiem') : I18n.t('GoodsDetailNew.finished'))}
											</Text>
                                            <TimeCountDown
                                        ...
                                        <View style={ [ styles.jtt_item_b ] }>
                                            <View style={ styles.jtt_line }>
                                                ...
                                                <View style={ styles.jtt_pro_l }>
                                                    <View style={ [ styles.jtt_pro, {width: `${ item.left_pro }%`} ] }/>
                                                </View>
                                                <View style={ styles.jtt_pro_r }>
                                                    <View style={ [ styles.jtt_pro, {width: `${ item.right_pro }%`} ] }/>
                                                </View>
                                            </View>
                                ...
                                <View style={ [ styles.sld_single_line, ] }>
                                    <View style={ styles.sld_single_left_view }>
                                        ...
                                        <View style={ [ styles.sld_single_left_text, GlobalStyles.sld_global_font ] }>
                                            ...
                                            <Text style={ [ styles.sld_single_left_text_detail, GlobalStyles.sld_global_font ] }/>
				...
				<View style={ {
					...
					>
						<Image
							...
							resizeMode={'contain'}
						/>
						<Text style={{color: '#2D2D2D',fontSize: language === LANGUAGE_MIANWEN ? 8 : 10, marginTop: pxToDp(8)}}>{I18n.t('GoodsDetailNew.service')}</Text>
					</TouchableOpacity>
					{/*跳转到购物车*/}
					{
						!(params && params.fromVip) ?
                            <TouchableOpacity style={ styles.bottomCommon } ctiveOpacity={ 1 } onPress={ () => this.goCart() }>
                                <Image
                                    ...
                                    resizeMode={'contain'}
                                />
                                <Text style={{color: '#2D2D2D',fontSize: language === LANGUAGE_MIANWEN ? 8 : 10, marginTop: pxToDp(8)}}>{I18n.t('GoodsDetailNew.shoppingcart')}</Text>
                                {
										...
										<View style={ {flexDirection: 'row', marginBottom: 25, flexWrap: 'wrap',} }>
											{
												items.goods_spec_value.map((val, indexchild) => {
													return (
														...
														>
                                                            <View style={[styles.spec_detail, val.default == 1 ? styles.spec_detail_sele : styles.spec_detail_nosele]}>
																<Text style={[GlobalStyles.sld_global_font, styles.spec_detail_text, {color: val.default == 1 ? '#e58011' : '#666'}]}>
                                                                    {val.specs_value_name}
																</Text>
															</View>
                                                        </TouchableOpacity>
													)
