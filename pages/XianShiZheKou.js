...
export default class XianShiZheKou extends Component {
	...
	constructor ( props ) {
		...
		this.state = {
			diy_data : [] ,//存放所有的装修改数据
			title : I18n.t('XianShiZheKou.Limitdiscounts') ,//页面标题
			data : [] ,//存放数据
			pn : 1 ,//当前页
			hasmore : true ,//是否还有数据
			refresh : false ,//是否刷新
	...
	//获取装修数据
	getDiyData = () => {
		RequestData.getSldData ( AppSldUrl + '/index.php?app=index&mod=topic&activity_type=xianshi'+this.extend_param_str)
			.then ( result => {
	...
	getGoodsInfo = ( type = 0 ) => {
		...
		RequestData.getSldData ( AppSldUrl + '/index.php?app=discount&mod=xianshi_goods_list&key=' + key + '&currentPage=' + pn+this.extend_param_str )
			.then ( result => {
				if ( result.code == 200 ) {
					if ( refresh ) {
						refresh = false;
	...
	footerView = () => {
		...
		if ( data.length > 0 || pn > 1 ) {
			content = <View style={ styles.flatlist_footer_view }><Text
				style={ styles.flatlist_footer_text }>{ hasmore ? I18n.t('com_PinTuan.Data_loading') : I18n.t('XianShiZheKou.Loadcompleted') }</Text></View>
		}
		return content;
	}
	...
	renderCell = ( item , index ) => {
		return (
			<TouchableOpacity
				...
				style={ styles.sld_xs_item_view }>
				{ ViewUtils.getSldImg ( 252 , 252 , { uri : item.goods_image_url } ) }

				<View style={ styles.sld_xs_rpart_view }>
					<Text numberOfLines={ 2 } style={ styles.sld_xs_rpart_gname }>{ item.goods_name }</Text>

					<View style={ styles.sld_xc_rpartb_view }>
						<View style={ [ styles.sld_xs_tip , GlobalStyles.flex_common_row ] }><Text
							style={ styles.sld_xs_tip_text }>{I18n.t('XianShiZheKou.Limitdiscounts')}</Text></View>
						<View style={ [ GlobalStyles.flex_common_row , { width : pxToDp ( 96 ) } ] }>
							<Text style={ styles.sld_xs_discount }>{ item.xianshi_discount }</Text>
						</View>
					</View>
					<View style={ styles.sld_xs_rbpart_bottom }>
						<View style={ styles.sld_xs_rbpart_wrap }>
							<Text style={ styles.sld_xs_gshow_price }>ks{PriceUtil.formatPrice( item.promotion_price )}</Text>
							<Text style={ styles.sld_xs_gmarket_price }>ks{PriceUtil.formatPrice( item.goods_marketprice )}</Text>
						</View>
						<View style={ styles.sld_xs_qiang_view }>
							<Text style={ styles.sld_xs_qiang_text }>{I18n.t('XianShiZheKou.Grabimmediately')}</Text>
						</View>
