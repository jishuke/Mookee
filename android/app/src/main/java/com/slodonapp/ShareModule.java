...
public class ShareModule extends ReactContextBaseJavaModule {
    ...
    private UMShareListener getUMShareListener(final Callback successCallback){
        return new UMShareListener() {
            ...
            public void onStart(SHARE_MEDIA share_media) {
                android.util.Log.e("xxxxxx1","start=");
            }

            @Override
            public void onResult(SHARE_MEDIA share_media) {
                            android.util.Log.e("xxxxxx2","result=");
    ...
    private UMImage getImage(String url){
        if (TextUtils.isEmpty(url)){
            ...
            Log.i("Mookee", newUrl);
            Bitmap bitmap = BitmapFactory.decodeFile(newUrl);
            return new UMImage(ma, bitmap);
        } else {
            return new UMImage(ma,url);
        }
