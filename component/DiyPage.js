...
export default class DiyPage extends Component {
	...
	render () {
		...
		console.log('type----------------:::', typeof diy_data)
		return ( <View>
				{ /*团购活动模版--start*/ }
							...
							<View style={ { flexDirection : 'row' , width : width , height : pxToDp ( 390 ) } }>
								{ typeof ( diy_data.data.bottom.left[ 1 ].goods_info[ 0 ].end_time ) == 'undefined' && (
									<View style={ {
										...
										<Text style={ { color : '#999' , fontSize : pxToDp ( 24 ) } }>{I18n.t('DiyPage.text1')}</Text></View>
								) }
								{ typeof ( diy_data.data.bottom.left[ 1 ].goods_info[ 0 ].end_time ) != 'undefined' && (
									<TouchableOpacity
										activeOpacity={ 1 }
											...
											<View style={ {
												...
												<View
													...
													<Text style={ {
														...
														fontWeight : '700' ,
														marginBottom : -pxToDp ( 5 )
													} }>{ PriceUtil.formatPrice(PriceUtil.formatPrice(diy_data.data.bottom.left[ 1 ].goods_info[ 0 ].promotion_price)) }</Text>
												</View>
												<View style={ [ GlobalStyles.flex_common_row , {
													...
													<Text style={ { color : '#fff' , fontSize : pxToDp ( 20 ) } }>{I18n.t('DiyPage.text2')}</Text>
												</View>
											</View>
										</View>
										<View
				...
				{ diy_data.type == 'lunbo' && (
					<Swiper
						...
						horizontal={ true }
						autoplay={ true }
                        onIndexChanged={(index)=>{ this.props.setTopColor && this.props.setTopColor(diy_data.data[index].lunbo_color)}}
                    >
				...
				{ /*轮播模版二 --start*/ }
				{ diy_data.type == 'lunbo2' && (
					<View style={styles.swiper_box2}>
						<View style={[styles.swiper_bgcolor, {backgroundColor: '#ff6666'}]}></View>
						<Swiper
							showsPagination={ false }
							style={ styles.swiper_diy2 }
							horizontal={ true }
							autoplay={ true }>
				...
				{ diy_data.type == 'fuwenben' && Platform.OS != 'ios' && (
					...
					>
						<WebView
							...
							onLoadEnd={ () => this._onLoadEnd(thisWebView) }
						/>
					</View>
				) }
				{ /*富文本模版-android-end*/ }
					...
					<View style={ [ styles.tjsp_small_wrap ] }>
						{ diy_data.data.goods_info != null && diy_data.data.goods_info.map ( ( val , index ) => (
							<TouchableOpacity key={ index } activeOpacity={ 1 }
							                  onPress={ () => {
							                  	console.warn('ww:tuijianshangpin', val);
																	ViewUtils.goGoodsDetail ( navigation , val.gid );
																}}
							                  key={ val.gid }>
								<View style={ styles.tjsp_small_part }>
                                    ...
                                    {/*佣金*/}
									{
										val.push_price && parseFloat(val.push_price) > 0 ?
                                        <View style={{flexDirection: 'row'}}>
                                            <View style={{marginTop: pxToDp(22), justifyContent: 'center', alignItems: 'center', backgroundColor: '#FAEDDA', borderRadius: 2.0}}>
                                                <Text style={{marginHorizontal: pxToDp(10), fontSize: pxToDp(20), color: '#FAA500'}}>{`${I18n.t('HomeScreenNew.commission')}: ${PriceUtil.formatPrice(val.push_price || '')}Ks`}</Text>
                                            </View>
                                            <View style={{flex: 1}}/>
                                        </View> : null
									}
				...
				{ diy_data.type == 'kefu' && diy_data.tel != '' && (
					<TouchableOpacity style={ styles.kefu_wrap } activeOpacity={ 1 }
					                  onPress={ () => ViewUtils.callTel ( diy_data.tel ) }
					>
					...
					<View>
						...
						<ScrollView horizontal={ true } style={ styles.dapei_scroller } showsHorizontalScrollIndicator={ false }>
							<View style={ styles.dapei_whole }>
								{ diy_data.data.goods_info && diy_data.data.goods_info.map ( ( item , index ) => (
				...
				{ diy_data.type == 'tupianzuhe' && diy_data.sele_style == 1 && (
					<View style={ styles.tpzh_1_wrap }>
						{ diy_data.data.map ( ( val , index ) => (
							<TouchableOpacity activeOpacity={ 1 } key={ 'tpzh' + index }
							                  onPress={ () => {
							                  	console.warn('ww:tupianzuhe', val);
																	ViewUtils.goDetailPage ( navigation , val.url_type == 'url' ? val.url_type_new : val.url_type , val.url_type == 'url' ? val.url_new : val.url );
																} }>
								<Image resizeMode='cover'
